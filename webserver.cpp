...
WebServer::WebServer()
{
  ...
  sprintf(m_root,"%s%s",server_path,root);
...
void WebServer::init(Config config)
{
  ...
  m_TRIGMode = config.CONNTrigmode;
...
{
  m_LISTENTrigmode= m_TRIGMode<2?0:1;
  m_CONNTrigmode=m_TRIGMode%2==0?0:1;
}
  ...
  if (!m_close_log)
    {
      //初始化日志
      if (m_log_write)
        Log::get_instance()->init("./.log", m_close_log, 2000, 800000, 800);
      else
        Log::get_instance()->init("./.log", m_close_log, 2000, 800000, 0);
    }
}
